...
def historical_transactions(num_rows=None):
    ...
    # from discussion with reddar
    for i in range (0, 14):
        hist_df['purchase_amount_lag_'+str(i)] = hist_df['purchase_amount'].apply(lambda x: x if hist_df['month_lag'] is i else 0)
    ...
    for i in range (0, 14):
        aggs['purchase_amount_lag_'+str(i)] = ['sum']
...
def additional_features(df):
    ...
    #purchase ratio
    for i in range (0, 13):
        df['ratio_purchase_amount_lag_'+str(i)] = df['new_purchase_amount_sum']/df['hist_purchase_amount_lag_'+str(i)+'_sum']
        df['ratio_purchase_amount_lag_'+str(i)+str(i+1)] = df['hist_purchase_amount_lag_'+str(i)+'_sum']/df['hist_purchase_amount_lag_'+str(i+1)+'_sum']
...
def ks_2(train_df,test_df):
    ...
    del list_discarded[0:2]
    return list_discarded
    
#feature elimination 2

def feature_sel(train_df,test_df):
    target = train_df['target']
    train_matrix = train_df.drop(['first_active_month','target', 'outliers',\
                                  'hist_purchase_date_max', 'hist_purchase_date_min', 'hist_card_id_size',\
                                  'new_purchase_date_max', 'new_purchase_date_min', 'new_card_id_size'], axis=1)
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    if stratified:
        folds = StratifiedKFold(n_splits= num_folds, shuffle=True, random_state=1233)
    else:
        folds = KFold(n_splits= num_folds, shuffle=True, random_state=1233)
...
def main(debug=False):
    ...
    with timer("feature selection 1"):
        list_discarded = ks_2(train_df,test_df)
        train_df = train_df.drop(list_discarded, axis=1)
        test_df = test_df.drop(list_discarded, axis=1)
    
    with timer("feature selection 2"):
        all_to_remove = feature_sel(train_df,test_df)
        train_df = train_df.drop(all_to_remove, axis=1)
        test_df = test_df.drop(all_to_remove, axis=1)
    with timer("Run LightGBM with kfold"):
        kfold_lightgbm(train_df, test_df, num_folds=11, stratified=True, debug=debug)
